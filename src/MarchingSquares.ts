...
export const MarchingSquares: any = {};

MarchingSquares.getBlobOutlinePoints = function (
  source_array: HTMLCanvasElement | Uint8Array,
  width: number,
  height: number = 0
) {
  ...
  const startingPoint = MarchingSquares.getFirstNonTransparentPixelTopDown(source_array, width, height);
...
MarchingSquares.getFirstNonTransparentPixelTopDown = function (
...
MarchingSquares.walkPerimeter = function (
  source_array: Uint8Array,
  width: number,
  height: number,
  start_w: number,
  ...
  const point_list = [],
    ...
    down = 3 | 0,
    right = 4 | 0,
    step_func = MarchingSquares.step;
...
MarchingSquares.step = function (idx: number, source_array: Uint8Array, width: number) {
  ...
  switch (state) {
    case 1:
      MarchingSquares.next_step = up;
      break;
    case 2:
      MarchingSquares.next_step = right;
      break;
    case 3:
      MarchingSquares.next_step = right;
      break;
    case 4:
      MarchingSquares.next_step = left;
      break;
    case 5:
      MarchingSquares.next_step = up;
      break;
    case 6:
      if (MarchingSquares.next_step == up) {
    ...
    case 7:
      ...
      break;
    case 8:
      MarchingSquares.next_step = down;
      break;
    case 9:
      if (MarchingSquares.next_step == right) {
        // info from previous_step
        MarchingSquares.next_step = up;
      } else {
        MarchingSquares.next_step = down;
    ...
    case 14:
      ...
      break;
    default:
      MarchingSquares.next_step = none; // this should never happen
      break;
  }
  return MarchingSquares.next_step;
};

// window.MarchingSquaresOpt = MarchingSquares;
